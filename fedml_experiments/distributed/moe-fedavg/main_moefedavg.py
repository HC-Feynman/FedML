...
from fedml_api.distributed.moefedavg.MoEFedAvgAPI import FedML_init, FedML_MoEFedAvg_distributed
...
def add_args(parser):
    ...
    """
    # Training settings
    parser.add_argument("--model", type=str, default="moe-cnn", metavar="N", help="neural network used in training")
    ...
    parser.add_argument("--num_experts_to_choose", type=int, default=2, help="k in MoE")
...
def create_model(args, model_name, output_dim, device):
    ...
    elif model_name == "moe-cnn" and args.dataset == "femnist":
        logging.info("MOE-CNN + FederatedEMNIST")
        model = {}
        model["gating"] = MoE_CNN_Gating(args, device)
        model["experts"] = [CNN_DropOut(False) for _ in range(args.num_experts)]
